...
import { noImg } from '../../utilities/constanats';
...
const PokemonInfo: React.FC = () => {
  ...
  const handleFavourites = async () => {
    if (currUser && id)
      await addFavourites(currUser, {
        [id]: 'https://pokeapi.co/api/v2/pokemon/' + id,
      });
  };

  const handleRemove = async () => {
    if (currUser && id) await removeFavourites(currUser, id);
  };
      ...
      <div className="info-div">
        <div className="poke-head">
          <div className="poke-img">
            <img
              src={`https://raw.githubusercontent.com/lordvader501/pokelister/test-white/src/assets/pokemons/shiny/${id}.png`}
              alt={id}
              onError={(event) => {
          ...
          <div className="name-fav-container">
            ...
            {currUser &&
              (id && !(id in currFav) ? (
                <button className="fav-button" onClick={handleFavourites}>
                  ➕ add to favourites
                </button>
              ) : (
                <button className="fav-button danger" onClick={handleRemove}>
